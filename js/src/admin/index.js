...
const SerpPreview = (item, type = 'web') => {
    return m('div', {style: 'background:#fff; border: 1px solid #dfe1e5; border-radius: 8px; padding: 15px; font-family: arial, sans-serif; max-width: 600px; margin-top:10px;'}, [
        m('div', {style: 'color: #202124; font-size: 14px; margin-bottom: 2px;'}, 'https://forum.spottersturkey.com › ...'),
...
class SeoAdminPage {
    oninit(vnode) {
        ...
        this.data = {
            ...
            cache: JSON.parse(localStorage.getItem('spottersturkey_seo_cache_v2')) || {}
        };
        this.loadDashboard();
    }
    ...
    saveCache(url, data) {
        ...
        localStorage.setItem('spottersturkey_seo_cache_v2', JSON.stringify(this.data.cache));
    }
        ...
        return m('div.SeoPack-Admin', [
            ...
            m('div.SeoContent', [
                ...
                this.activeTab === 'images' ? m('div.SeoCard', [
                    m('h3', 'Görsel SEO Denetimi [spotter-image]'),
                ...
                // 4. AYARLAR (TÜM ORİJİNAL ALANLAR GERİ DÖNDÜ)
                this.activeTab === 'settings' ? m('div.SeoCard', [
                    m('h3', 'Google Servisleri ve Temel Ayarlar'),
                    m('div.Form-group', [ m('label', 'Google Analytics 4 Ölçüm Kimliği (G-XXXXXXXX)'), m('input.FormControl', { bidi: this.setting('spottersturkey-seo.ga_id') }) ]),
                    m('div.Form-group', [ m('label', 'GA4 Mülk Kimliği (Sayısal ID - Dashboard İçin)'), m('input.FormControl', { bidi: this.setting('spottersturkey-seo.ga_id_number') }) ]),
                    m('div.Form-group', [ m('label', 'Google Search Console Doğrulama Kodu'), m('input.FormControl', { bidi: this.setting('spottersturkey-seo.gsc_code') }) ]),
                    m('div.Form-group', [ m('label', 'Google Ads Yayıncı Kimliği (ca-pub-XXXXXXXX)'), m('input.FormControl', { bidi: this.setting('spottersturkey-seo.ads_client') }) ]),
                    m('div.Form-group', [ m('label', 'Site Açıklaması (Meta Description)'), m('textarea.FormControl', { bidi: this.setting('spottersturkey-seo.meta_description_prefix') }) ]),
                    m('div.Form-group', [ m('label', 'Meta Anahtar Kelimeler'), m('textarea.FormControl', { bidi: this.setting('spottersturkey-seo.meta_keywords') }) ]),
                    m('div.Form-group', [ m('label', 'Cron-Job.org Gizli Anahtarı (Tetikleme Şifresi)'), m('input.FormControl', { bidi: this.setting('spottersturkey-seo.trigger_secret') }) ])
                ]) : null
            ])
        ]);
    }
...
app.initializers.add('spottersturkey-seo-pack', () => {
    app.extensionData.for('spottersturkey-seo-pack').registerSetting(function () {
