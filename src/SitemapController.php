...
class SitemapController implements RequestHandlerInterface
{
    ...
    {
        if (!$this->settings->get('spottersturkey-seo.enable_sitemap')) {
        ...
        $posts = Post::where('type', 'comment')
            ...
            ->where(function($query) {
                $query->where('content', 'LIKE', '%[spotter-image%')
        ...
        foreach ($posts as $post) {
            ...
            if (!empty($imgSrcs)) {
                ...
                $altText = null;
                if (preg_match('/\[spotter-image[^\]]+alt=(?:"([^"]*)"|([^\]]+))/i', $post->content, $altMatches)) {
    ...
    private function extractAllImageSrcs($post, $baseUrl) {
        ...
        if (preg_match_all('/\[spotter-image[^\]]+url=["\']?([^"\'\]]+\.(?:jpg|jpeg|png|webp))["\']?/i', $post->content, $matches)) {
