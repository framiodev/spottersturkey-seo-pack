...
class InjectSeoTags
{
    ...
    public function __invoke(Document $document, ServerRequestInterface $request)
    {
        ...
        $gaId = $this->settings->get('spottersturkey-seo.ga_id'); 
    ...
    private function injectTagTags(Document $document, ServerRequestInterface $request, $tagSlug)
    {
        ...
        // Tag modelini çekmeye çalışalım (Tags eklentisi yüklü olmalı)
        $tag = null;
    ...
    private function injectDiscussionTags(Document $document, $request, $discussionId, $postNumber)
    {
        try {
            ...
            $jsonLd = [
                ...
                'author' => [
                    '@type' => 'Person',
                    'name' => $discussion->user ? $discussion->user->display_name : 'Spotters Turkey',
            ...
            foreach ($allImages as $image) {
                ...
                $brandInfo = ' - ' . $this->settings->get('forum_title') . ' (forum.spottersturkey.com)';
                ...
                if (!empty($imgCaption)) {
                    ...
                    $imgKeywords = implode(', ', $parts) . ', Spotters Turkey, Otobüs Fotoğrafları';
                }
            ...
            // Eğer varsa etiketleri de breadcrumb'a ekleyelim
            ...
            $document->head[] = '<!-- SEO PACK ERROR: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES) . ' in ' . $e->getFile() . ':' . $e->getLine() . ' -->';
        }
    ...
    private function extractAllImages($post, $baseUrl) {
        ...
        // 1. [spotter-image] veya [upl-image-preview] taraması
        if (preg_match_all('/\[(?:spotter-image|upl-image-preview).*?\]/is', $content, $matches)) {
            foreach ($matches[0] as $tag) {
                $url = '';
                $alt = '';
                
                // URL içinde boşluk olabileceği için \s engellememeliyiz.
                ...
                // Alt etiketi için: eğer çift tırnak içindeyse tek tırnak engel olmamalı.
    ...
    private function extractDescription($post) {
        // [spotter-image ... ] etiketini temizle
        $raw = preg_replace('/\[spotter-image.*?\]/s', '', $post->content);
        // URL taglerini temizle
