...
class BatchIndexController implements RequestHandlerInterface
{
    ...
    {
        try {
            $lastIndex = (int) $this->settings->get('spottersturkey-seo.last_batch_index', 0);
            $batchSize = 50;
            ...
            $query = CommentPost::where('is_private', false)
                ->whereNull('hidden_at')
                ->where(function($q) {
                    $q->where('content', 'like', '%spotter-image%')
            ...
            if ($posts->isEmpty()) {
                $this->settings->set('spottersturkey-seo.last_batch_index', 0);
            ...
            $this->settings->set('spottersturkey-seo.last_batch_index', $newIndex);
    ...
    protected function fixAltTags($post) {
        ...
        if (strpos($xml, 'SPOTTER-IMAGE') === false && strpos($xml, 'UPL-IMAGE-PREVIEW') === false) {
            return;
        }
        ...
        $xml = preg_replace_callback('/<(SPOTTER-IMAGE|UPL-IMAGE-PREVIEW)\s([^>]+)>/', function($m) use ($newAltText, $discussionTitle, $forumUrl, &$changed) {
            $tagName = $m[1];
            $attrs = $m[2];
